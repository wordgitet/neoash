...
This copy is vendored into freebsd-sh as a POSIX-focused test suite.

Local runner:

    TESTEE=../../build/src/sh ./run-posix.sh

The local wrapper requires a system `yash` available in `PATH`. It does not
use a hardcoded absolute path to the yash binary.

Make targets:

    make TESTEE=../../build/src/sh test-posix
